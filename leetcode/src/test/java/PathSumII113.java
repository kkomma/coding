...
public class PathSumII113 {
    public List<List<Integer>> pathSum(TreeNode3 root, int sum) {
        List<List<Integer>> res = new ArrayList<>();
        pathSum(root, 0, sum, new ArrayList<>(), res);
        return res;
    }

    private void pathSum(TreeNode3 root, int sumSoFar, int target, List<Integer> path, List<List<Integer>> res) {
        if (root == null) return;
