...
public class AllNodesDistanceKInBinaryTree863 {
    ...
    private int distanceToRoot(TreeNode3 root, TreeNode3 target, int K, List<Integer> res, int level) {
    ...
    private void distanceKChildren(TreeNode3 root, int K, List<Integer> res) {
        if (root == null) return;
     ...
     */
    Map<TreeNode3, TreeNode3> parent;
    public List<Integer> distanceK2(TreeNode3 root, TreeNode3 target, int K) {
        parent = new HashMap();
        dfs(root, null);
        ...
        Set<TreeNode3> seen = new HashSet();
        seen.add(target);
        seen.add(null);
    ...
    public void dfs(TreeNode3 node, TreeNode3 par) {
        if (node != null) {
            parent.put(node, par);
            dfs(node.left, node);
            dfs(node.right, node);
    ...
    TreeNode3 target;
    int K;
    public List<Integer> distanceK3(TreeNode3 root, TreeNode3 target, int K) {
        ans = new LinkedList();
        this.target = target;
        this.K = K;
        dfs(root);
    ...
    public void subtree_add(TreeNode3 node, int dist) {
        if (node == null) return;
        if (dist == K)
            ans.add(node.val);
        else {
    ...
    Map<TreeNode3, Integer> map = new HashMap<>();
    public List<Integer> distanceK4(TreeNode3 root, TreeNode3 target, int K) {
    ...
    private int find(TreeNode3 root, TreeNode3 target, int K) {
        if (root == null) return -1;
        if (root == target) {
            map.put(root, 0);
            return 0;
    ...
    private void dfs(TreeNode3 root, TreeNode3 target, int K, int length, List<Integer> res) {
        if (root == null) return;
