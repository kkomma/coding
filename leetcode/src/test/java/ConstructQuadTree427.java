...
class Node {
    ...
    public Node(boolean _val,boolean _isLeaf,Node5 _topLeft,Node5 _topRight,Node5 _bottomLeft,Node5 _bottomRight) {
        val = _val;
        isLeaf = _isLeaf;
        topLeft = _topLeft;
        topRight = _topRight;
...
public class ConstructQuadTree427 {
    public Node5 construct(int[][] grid) {
        if (grid == null) return null;
        int N = grid.length;
        return construct(grid, 0, N-1, 0, N-1);
    }

    public Node5 construct(int[][] grid, int xi, int xj, int yi, int yj) {
