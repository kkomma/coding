...
public class BinaryTreeLongestConsecutiveSequence298 {
    public int longestConsecutive(TreeNode3 root) {
        if (root == null) return 0;
        return helper(root, 0);
    }

    public int helper(TreeNode3 root, int i) {
    ...
    public int longestConsecutive2(TreeNode3 root) {
        dfs(root);
        return maxLength;
    }
    
    private int dfs(TreeNode3 p) {
        if (p == null) return 0;
        int L = dfs(p.left) + 1;
        int R = dfs(p.right) + 1;
