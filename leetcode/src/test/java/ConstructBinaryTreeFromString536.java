...
public class ConstructBinaryTreeFromString536 {
    public TreeNode3 str2tree(String s) {
        if (s == null || s.length() == 0) return null;
        return str2tree(s.toCharArray(), 0, s.length()-1);
    }
    
    public TreeNode3 str2tree(char[] chars, int i, int j) {
        if (i > j) return null;
        int len = 0;
        while (i+len <= j && chars[i+len] != '(' && chars[i+len] != ')') {
            len++;
        }
        TreeNode3 root = new TreeNode3(Integer.parseInt(new String(chars, i, len)));
