...
public class KthSmallestElementInABST230 {
    public int kthSmallest(TreeNode3 root, int k) {
        Stack<Integer> st = new Stack<>();
        kthSmallest(root, k, st);
        return st.pop();
    }

    public void kthSmallest(TreeNode3 root, int k, Stack<Integer> st) {
        if (root == null) return;
    ...
    public int kthSmallest2(TreeNode3 root, int k) {
        int[] i = new int[]{0, 0};
        kthSmallest(root, k, i);
        return i[1];
    }

    public void kthSmallest(TreeNode3 root, int k, int[] i) {
    ...
    public int kthSmallest3(TreeNode3 root, int k) {
        int l = kth(root.left);
        if (l >= k) {
