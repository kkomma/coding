...
public class SerializeAndDeserializeBST449 {
    ...
    class Codec {
        ...
        public TreeNode3 deserialize(String data) {
            String[] strs = data.split(",");
            TreeNode3 root = strs[0].equals("#") ? null : new TreeNode3(Integer.valueOf(strs[0]));
            Queue<TreeNode3> q = new LinkedList<>();
            q.add(root);
            int i = 1;
            while (i<=strs.length-2) {
                TreeNode3 p = q.poll();
                if (p != null) {
                    TreeNode3 left = strs[i].equals("#") ? null : new TreeNode3(Integer.valueOf(strs[i]));
                    TreeNode3 right = strs[i+1].equals("#") ? null : new TreeNode3(Integer.valueOf(strs[i+1]));
                    p.left = left;
                    p.right = right;
                    q.add(left);
                    q.add(right);
    ...
    class Codec2 {
        // Encodes a tree to a single string.
        public String serialize2(TreeNode3 root) {
            if (root == null) return "";
            StringBuilder sb = new StringBuilder();
            serialize(root, sb);
            return sb.substring(0, sb.length()-1).toString();
        ...
        private void serialize(TreeNode3 root, StringBuilder sb) {
            if (root == null) return;
            sb.append(root.val + ",");
        ...
        // Decodes your encoded data to tree.
        public TreeNode3 deserialize2(String data) {
            String[] strs = data.split(",");
            if (strs.length == 0 || strs[0].equals("")) return null;
            return deserialize(strs, 0, strs.length-1);
        }
        
        private TreeNode3 deserialize(String[] strs, int lo, int hi) {
            if (lo > hi) return null;
            if (lo == hi) return new TreeNode3(Integer.valueOf(strs[lo]));
            int rootVal = Integer.valueOf(strs[lo]);
            TreeNode3 root = new TreeNode3(Integer.valueOf(strs[lo]));
    ...
    class Codec3 {
        ...
        //  3 6
        // 2   7
        private TreeNode3 getNode(Queue<Integer> q) { //q: 5,3,2,6,7
            if (q.isEmpty()) return null;
            TreeNode3 root = new TreeNode3(q.poll());//root (5)
    ...
    class Codec4 {
        ...
        private TreeNode3 deserialize(String[] nodes, int[] index, int max) {
            if (index[0] >= nodes.length || Integer.valueOf(nodes[index[0]]) >= max) {
                return null;
            }
            TreeNode3 root = new TreeNode3(Integer.valueOf(nodes[index[0]++]));
    ...
    class Codec5 {
        ...
        private TreeNode3 deserialize(String[] nodes, int[] index, int min) {
