...
public class FindBottomLeftTreeValue513 {
    public int findBottomLeftValue(TreeNode3 root) {
        int res = root.val;
        Queue<TreeNode3> q = new LinkedList<>();
        q.add(root);

        while (!q.isEmpty()) {
            Queue<TreeNode3> ql = new LinkedList<>();
    ...
    public int findBottomLeftValue2(TreeNode3 root) {
        int res = root.val;
    ...
    private int helper(TreeNode3 node, int level, Map<Integer, Integer> lefts) {
     ...
     */
    public int findBottomLeftValue3(TreeNode3 root) {
        return findBottomLeftValue(root, 1, new int[]{0,0});
    }
    public int findBottomLeftValue(TreeNode3 root, int depth, int[] res) {
