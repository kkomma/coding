...
public class BinaryTreeZigzagLevelOrderTraversal103 {
    public List<List<Integer>> zigzagLevelOrder(TreeNode3 root) {
        ...
        LinkedList<TreeNode3> level = new LinkedList<>();
        level.add(root);
        zigzagLevelOrder(level, true, res);
        return res;
    }

    private void zigzagLevelOrder(LinkedList<TreeNode3> level, boolean left, List<List<Integer>> res) {
        if (level.isEmpty()) return;
        LinkedList<Integer> one = new LinkedList<>();
        LinkedList<TreeNode3> newLevel = new LinkedList<>();
        while (!level.isEmpty()) {
            TreeNode3 t = level.removeFirst();
            if (t == null) continue;
