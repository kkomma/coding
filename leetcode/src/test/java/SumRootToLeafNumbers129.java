...
public class SumRootToLeafNumbers129 {
    public int sumNumbers1(TreeNode4 root) {
        if (root == null) {
            return 0;
        }
    ...
    public int helper(TreeNode4 node, List<Integer> path, int sum) {
        if (node == null) {
            return sum;
        }
     ...
     */
    public int sumNumbers(TreeNode4 root) {
    	 return sum(root, 0);
    }

    public int sum(TreeNode4 n, int s){
    	  if (n == null) return 0;
